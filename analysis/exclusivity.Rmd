...
```
marker+ cell = <marker>_sign is "+"  (is_pos accepts +, pos, positive, yes, true, 1)
...
# dev = c("png", "pdf") chunk option) into output/figures/exclusivity/. Runs last so
# that all figures from the chunks above already exist on disk.
export_pdf_figures("exclusivity")
```
