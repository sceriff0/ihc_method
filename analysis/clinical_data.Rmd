...
neo_ids <- norm_slide_id(neoplastic_data$SAMPLE)
...
ihc_ids       <- unique(norm_slide_id(ihc_data$patient_id))
have_geojson  <- unique(norm_slide_id(annots$patient_id))
...
path_long <- neoplastic_data |>
  mutate(patient_id = norm_slide_id(SAMPLE)) |>
...
clin <- clinical_data |>
  mutate(patient_id = norm_slide_id(`ID PATIENT`)) |>
  select(patient_id,
